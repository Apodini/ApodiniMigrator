...
public enum DateEncodingStrategy: String, Codable, Hashable {
    ...
    init(from strategy: JSONEncoder.DateEncodingStrategy) {
        switch strategy {
        case .deferredToDate:
            self = .deferredToDate
        case .secondsSince1970:
    ...
    var toJSONEncoderStrategy: JSONEncoder.DateEncodingStrategy {
        switch self {
        case .deferredToDate:
...
public enum DataEncodingStrategy: String, Codable, Equatable {
    ...
    init(from strategy: JSONEncoder.DataEncodingStrategy) {
        switch strategy {
        case .deferredToData:
            self = .deferredToData
        case .base64:
    ...
    /// Corresponding strategy of `JSONEncoder`
    var toJSONEncoderStrategy: JSONEncoder.DataEncodingStrategy {
        switch self {
        case .deferredToData:
