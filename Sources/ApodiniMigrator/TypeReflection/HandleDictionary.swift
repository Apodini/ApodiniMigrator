...
func handleDictionary(_ node: Node<ReflectionInfo>) throws -> Node<ReflectionInfo> {
    let typeInfo = node.value.typeInfo

    guard typeInfo._mangledName == .dictionary,
    ...
    let newReflectionInfo = ReflectionInfo(
        ...
        cardinality: .zeroToMany(.dictionary(key: keyNode.value, value: valueNode.value))
    )
